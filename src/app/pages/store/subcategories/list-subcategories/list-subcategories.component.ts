...
export class ListSubcategoriesComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    if (this.auth.hasRole(['SUPERADMIN'])) {
      this.cols.splice(3, 0, { field: "tenant", header: "Gym" });
    }
  ...
  getList() {
    ...
    this.crud.getList('subcategories').subscribe((response: any) => {
  ...
  confirmDelete(item: any) {
    this.crud.confirmDelete(item, 'subcategories');
  }
  
  delete(item: any) {
    this.crud.delete(item, 'subcategories');
  }
